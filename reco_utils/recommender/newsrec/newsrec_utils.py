...
from reco_utils.recommender.deeprec.deeprec_utils import flat_config, load_yaml
import random
import re

from reco_utils.recommender.hparams import HParams
...
def check_nn_config(f_config):
    ...
    if f_config["model_type"] in ["nrms", "NRMS"]:
        ...
        raise ValueError("Model type must be one of nrms, naml, lstur, npa")
    ...
    if f_config["model_type"] in ["nrms", "NRMS", "lstur", "LSTUR"]:
        if f_config["data_format"] != "news":
            raise ValueError(
                "For nrms and naml trainer, data format must be 'news', but your set is {0}".format(
        ...
        if f_config["data_format"] != "naml":
            raise ValueError(
                "For nrms and naml trainer, data format must be 'naml', but your set is {0}".format(
                    f_config["data_format"]
                )
            )
