...
from reco_utils.recommender.dataset.download_utils import maybe_download
...
def check_nn_config(f_config):
    ...
    if f_config["model_type"] in ["exDeepFM", "xDeepFM"]:
        if f_config["data_format"] != "ffm":
            raise ValueError(
                "For xDeepFM trainer, data format must be 'ffm', but your set is {0}".format(
        ...
        if f_config["data_format"] != "dkn":
            raise ValueError(
                "For dkn trainer, data format must be 'dkn', but your set is {0}".format(
                    f_config["data_format"]
                )
            )
    check_type(f_config)
